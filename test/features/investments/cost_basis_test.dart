...
void main() {
  group('computeInvestmentEvents — 평단가 (SPEC §3.10)', () {
    test('매수는 잔액 영향 0, 평단만 누적', () {
      final events = computeInvestmentEvents([
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000),
      ]);
    ...
    test('부분 매도: 실현손익 = 매도금 - 평단원가', () {
      ...
      final events = computeInvestmentEvents([
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000),
        _e(id: 2, side: InvestmentSide.sell, on: '2025-01-02', qty: 4, total: 600),
      ]);
    ...
    test('여러 매수의 가중 평단', () {
      ...
      final events = computeInvestmentEvents([
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000),
        _e(id: 2, side: InvestmentSide.buy, on: '2025-01-02', qty: 10, total: 2000),
        _e(id: 3, side: InvestmentSide.sell, on: '2025-01-03', qty: 10, total: 2000),
      ]);
      final sell = events.firstWhere((e) => e.side == InvestmentSide.sell);
    ...
    test('보유수량보다 많이 매도하면 보유분까지만 원가 계산', () {
      ...
      final events = computeInvestmentEvents([
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 5, total: 500),
        _e(id: 2, side: InvestmentSide.sell, on: '2025-01-02', qty: 10, total: 1500),
      ]);
      final sell = events.firstWhere((e) => e.side == InvestmentSide.sell);
    ...
    test('배당은 전액 잔액 증가', () {
      final events = computeInvestmentEvents([
        _e(id: 1, side: InvestmentSide.dividend, on: '2025-01-05', qty: 0, total: 300),
      ]);
  ...
  group('eventsForAccount / investmentBalanceImpact', () {
    test('지정 자산의 이벤트만, 잔액 영향 합산', () {
      final all = [
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000, accountId: 1),
        _e(id: 2, side: InvestmentSide.sell, on: '2025-01-02', qty: 4, total: 600, accountId: 1),
        _e(id: 3, side: InvestmentSide.dividend, on: '2025-01-03', qty: 0, total: 300, accountId: 2),
      ];
    ...
    test('최신순 정렬 (occurredOn DESC)', () {
      final all = [
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000),
        _e(id: 2, side: InvestmentSide.dividend, on: '2025-03-01', qty: 0, total: 50),
      ];
  ...
  group('realizedPnL', () {
    final all = [
      _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000),
      _e(id: 2, side: InvestmentSide.sell, on: '2025-02-10', qty: 5, total: 700),
      _e(id: 3, side: InvestmentSide.dividend, on: '2025-02-15', qty: 0, total: 100),
      _e(id: 4, side: InvestmentSide.sell, on: '2025-03-20', qty: 5, total: 400),
    ];

    test('기간 내 매도·배당만, 매수 제외', () {
      final rows = realizedPnL(all, from: '2025-02-01', to: '2025-02-28');
      expect(rows.length, 2);
      expect(rows.every((r) => r.kind != RealizedKind.dividend || r.pnl == 100), true);
    });
  ...
  group('heldTickers', () {
    test('보유수량 > 0 인 종목만 정렬 반환', () {
      final all = [
        _e(id: 1, side: InvestmentSide.buy, on: '2025-01-01', qty: 10, total: 1000, ticker: 'TSLA'),
        _e(id: 2, side: InvestmentSide.sell, on: '2025-01-02', qty: 4, total: 600, ticker: 'TSLA'),
        _e(id: 3, side: InvestmentSide.buy, on: '2025-01-03', qty: 5, total: 500, ticker: 'AAPL'),
        _e(id: 4, side: InvestmentSide.sell, on: '2025-01-04', qty: 5, total: 700, ticker: 'AAPL'),
      ];
